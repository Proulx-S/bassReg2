function [cmd,fig] = qaReg4(xSet,ppLabel,acqLabel,xFlag,force)
...
for s = 1:length(xSet)
    ...
    switch ppLabel
        case 'full'
            fBefore = cat(1,fBefore,xSet{s}.initFiles.fPlumbList(:,1));
            bidsList = xSet{s}.initFiles.bidsList(:,1,:);
            bids = cell(size(xSet{s}.initFiles.fPlumbList(:,1)));
            
            for f = 1:size(bidsList,1)
                bids{f,1} = strjoin(bidsList(f,1,:),'_');
            end
            fBidsBefore = cat(1,fBidsBefore,bids);
            
            fAfter  = cat(1,fAfter ,xSet{s}.finalFiles.fPreprocList(:,1));
            bidsList = xSet{s}.finalFiles.bidsList(:,1,:);
            bids = cell(size(xSet{s}.finalFiles.fPreprocList(:,1)));
            for f = 1:size(bidsList,1)
                bids{f,1} = strjoin(bidsList(f,1,:),'_');
            end
...
[im,imMask,cmd.allBefore,cmd.allFMLbefore,cmd.allMeansBefore] = summarizeData(fBefore,mList,0,outDir,fBidsBefore,'before',force);
...
[im,imMask,cmd.allAfter,cmd.allFMLAfter,cmd.allMeansAfter] = summarizeData(fAfter,mList,0,outDir,fBidsAfter,'after',force);
...
function [im,imMask,cmdAll,cmdFML,cmdMeans] = summarizeData(fList,mList,dummy,outDir,fBids,ppLabel,force)
global srcFs
xFlag = 0;
if ~exist('force','var'); force = []; end
if isempty(force);        force = 0 ; end
...
if force || ~exist(fMeans,'file')
    ...
    for f = 1:length(fList)
        ...
        if mri.nframes>1
            ind = [dummy(f)+1 floor(mean([dummy(f) mri.nframes])) mri.nframes];
            mri.vol = mri.vol(:,:,:,ind);
            % out = fullfile(outDir,'fml'); if ~exist(out,'dir'); mkdir(out); end
            % fFML{f} = fullfile(out,[ppLabel '_' fBids{f} '.nii.gz']);
