...
taskList = fields(rCond); taskList(~startsWith(taskList,'task_')) = [];
acq     = {};
bidsDir = {};
date    = [];
fBefore = [];
...
for t = 1:length(taskList)
    if isempty(rCond.(taskList{t}).fList); continue; end
    acq     = cat(1,acq    ,repmat(cellstr(rCond.(taskList{t}).acq)         ,size(rCond.(taskList{t}).fList,1),1));
    bidsDir = cat(1,bidsDir,repmat(cellstr(rCond.(taskList{t}).info.bidsDir),size(rCond.(taskList{t}).fList,1),1));
    date    = cat(1,date   ,rCond.(taskList{t}).acqTime                                                          );
    % date    = cat(1,date   ,rCond.(taskList{t}).date(1) + rCond.(taskList{t}).acqTime                            );
    fBefore = cat(1,fBefore,rCond.(taskList{t}).fOrigList(:,1)                                                   );
    fAfter  = cat(1,fAfter ,rCond.(taskList{t}).fPreprocList(:,1)                                                );
    % tmp = rCond.(taskList{t}).fPreprocMaskList;  for r = 1:size(tmp,1); tmp(r,1) = tmp(r,find(~cellfun('isempty',tmp(r,:)),1,'last')); end; tmp(:,2:end) = [];
    % fMask   = cat(1,fMask  ,tmp                                                                                  );
    fMask = cat(1,fMask,rCond.(taskList{t}).fPreprocMaskList);
    tmp = rCond.(taskList{t}).bidsList(:,1,2:end); for r = 1:size(tmp,1); tmp{r,1} = strjoin(tmp(r,:),'_');                              end; tmp(:,2:end) = [];
    bidsName   = cat(1,bidsName  ,tmp                                                                            );
    nFrame  = cat(1,nFrame,rCond.(taskList{t}).nFrame                                                            );
    nDummy  = cat(1,nDummy,rCond.(taskList{t}).nDummy                                                            );
    
    info = rCond.(taskList{t}).info;
end
[~,b] = sort(date);
acq     = acq(b);
bidsDir = bidsDir(b);
...
fAfter  = fAfter(b);
fMask   = fMask(b,:,:);
% fMask   = repmat(fMask,size(fBefore,1),1,1);
bidsName = bidsName(b);
nFrame  = nFrame(b);
nDummy  = nDummy(b);

acq     = unique(acq);     if length(acq)>1;     dbstack; error('X'); end; acq     = char(acq);
bidsDir = unique(bidsDir); if length(bidsDir)>1; dbstack; error('X'); end; bidsDir = char(bidsDir);
                           % if length(unique(fMask))>1;   dbstack; error('X'); end
[~,bidsSubDir] = fileparts(fileparts(fBefore{1}));

outDir = fullfile(bidsDir,['sub-' info.sub],'ses-cat',bidsSubDir,'derivatives',['set-' acq],'qaReg-full');
...
% if force || ~exist(outDir,'dir')

[im,imMask,cmd.allBefore,cmd.allFMLbefore,cmd.allMeansBefore] = summarizeData(fBefore,fMask,nDummy,outDir,bidsName,'before',force);
imMask = all(imMask,5);
if size(imMask,3)>1; imMask(:,:,[1 end],:,:) = 1; end
fig.fBefore = correlateFrames(im,logical(1-imMask),info.sub,acq,'full',nFrame+nDummy,bidsName,'before',outDir,force);

%% Summarize after after
[im,~,cmd.allAfter,cmd.allFMLAfter,cmd.allMeansAfter] = summarizeData(fAfter,fMask,[],outDir,bidsName,'after',force);
fig.fAfter = correlateFrames(im,logical(1-imMask),info.sub,acq,'full',nFrame,bidsName,'after',outDir,force);



tmp = strsplit(cmd.allAfter{end},newline); tmp = strsplit(tmp{end},' ');
smr.runAv.fList  = tmp(startsWith(tmp,filesep))';
smr.runSd    = [];
smr.runSnr   = [];
tmp = strsplit(cmd.allFMLAfter{end},newline); tmp = strsplit(tmp{end},' ');
smr.runFML.fList = tmp(startsWith(tmp,filesep))';
tmp = strsplit(cmd.allMeansAfter,newline); tmp = strsplit(tmp{end},' ');
smr.sesCat.runAv.fList  = tmp(startsWith(tmp,filesep))';
smr.sesCat.runSd  = [];
smr.sesCat.runSnr = [];
smr.sesCat.runFML = [];
smr.sesAv.runAv  = [];
...
smr.sesAv.runSnr = [];
smr.sesAv.runFML = [];
smr.prcMasks.f  = fMask;
smr.prcMasks.im = imMask;
...
nFrame    = {};
outDir    = {};
if length(xSet)>1; dbstack; error('should be only one set???'); end
for s = 1:length(xSet)
    subList{end+1} = {};
    labelList{end+1} = {};
    ...
    if isempty(xSet{1}.fList)
        nFrame{end+1,1}  = 0;
        outDir{end+1,1}  = [];
        continue
    end
    nFrame{end+1,1}  = xSet{s}.initFiles.nFrame;
    [~,bidsSubDir] = fileparts(fileparts(xSet{s}.fOrigList{1}));
    outDir{end+1,1}    = fullfile(xSet{s}.info.bidsDir,['sub-' xSet{s}.info.sub],['ses-' xSet{s}.info.ses],bidsSubDir,'derivatives',['set-' xSet{s}.label]);
...
outDir = fullfile(char(outDir),['qaReg-' ppLabel]); if ~exist(outDir,'dir'); mkdir(outDir); end
...
[im,imMask,cmd.allBefore,cmd.allFMLbefore,cmd.allMeansBefore] = summarizeData(fBefore,mList,dummy,outDir,fBidsBefore,'before',force);
if size(imMask,3)>1; imMask(:,:,[1 end]) = 1; end
fig.fBefore = correlateFrames(im,logical(1-imMask),subList,labelList,ppLabel,nFrame,fBidsBefore,'before',outDir,force);

%% Summarize after after
[im,imMask,cmd.allAfter,cmd.allFMLAfter,cmd.allMeansAfter] = summarizeData(fAfter,mList,dummy,outDir,fBidsAfter,'after',force);
fig.fAfter = correlateFrames(im,logical(1-imMask),subList,labelList,ppLabel,nFrame,fBidsAfter,'after',outDir,force);


function out = correlateFrames(im,imMask,subList,labelList,ppLabel,nFrame,fBids,ppLabel2,outDir,force)
if ~exist('ppLabel2','var'); ppLabel2 = []; end
if ~exist('force','var'); force = []; end
if isempty(force);        force = 0 ; end

out = fullfile(outDir,[ppLabel '_' ppLabel2 '_xFrameCorrelation.fig']);

if isempty(im) || isempty(imMask); return; end
if exist(out,'file') && ~force; disp([out ' already exists']); return; end


% correlate frames
for r = 1:length(im)
    im{r} = permute(im{r},[4 1 2 3]);
    im{r} = permute(im{r}(:,imMask),[2 1]);
end
im = cat(2,im{:});
...
title(strjoin([subList labelList {ppLabel} {ppLabel2}],'; '))
...
if length(nFrame)>1
    sLine = cumsum(nFrame)'; sLine = [0 sLine(1:end-1)]+0.5;
...
function [im,imMask,cmdAll,cmdFML,cmdMeans] = summarizeData(fList,mList,dummy,outDir,bidsName,ppLabel,force)
global srcFs
xFlag = 0;
if ~exist('force','var'); force = []; end
if isempty(force);        force = 0 ; end
...
out = fullfile(outDir,[ppLabel '_fml']); if ~exist(out,'dir'); mkdir(out); end
fFML    = cell(size(fList));
for f = 1:length(fList)
    fFML{f} = fullfile(out,[bidsName{f} '.nii.gz']);
end

% do = force; if ~exist(fMeans,'file'); do = 1; end

if force || ~exist(fMeans,'file')
    ...
    if isempty(dummy); dummy = zeros(size(mriMean)); end
    im      = cell(size(fList));
    imMask  = false([mriHd.volsize 1             length(fList)]);
    for f = 1:length(fList)
        disp(['reading and summarizing (' num2str(f) '/' num2str(length(fList)) ') ' fList{f} ])
        %read
        mri     = MRIread(fList{f});
        mriMask = MRIread(mList{f,1,1});
        ...
        if mri.nframes>1
            ind = [dummy(f)+1 floor(mean([dummy(f)+1 mri.nframes])) mri.nframes];
            mri.vol = mri.vol(:,:,:,ind);
    ...
    disp('reading and summarizing already done, skipping')
    im = [];
    imMask = [];
end
